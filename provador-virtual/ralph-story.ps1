...
# Determine project root (go up 1 level from this script's directory)
# Script is in: E:\Backup_HD\code\provador-virtual
# We need: E:\Backup_HD\code
...
$STORIES_DIR   = Join-Path $projectRoot "docs/stories"
...
# Keep absolute path for file existence checking
$StoryFileAbsolute = $StoryFile
if (-not [System.IO.Path]::IsPathRooted($StoryFile)) {
    # If relative, make it absolute relative to project root
    $StoryFileAbsolute = Join-Path $projectRoot $StoryFile
}
...
if ([System.IO.Path]::IsPathRooted($StoryFile)) {
    ...
    $itemPath = [System.IO.Path]::GetFullPath($StoryFile)
    $basePath = [System.IO.Path]::GetFullPath($projectRoot)
    if ($itemPath.StartsWith($basePath)) {
        $StoryFile = $itemPath.Substring($basePath.Length).TrimStart('\')
    }
}
...
if (-not (Test-Path $StoryFileAbsolute -PathType Leaf)) {
    Write-Err "Story file not found: $StoryFileAbsolute"
    exit 2
}
