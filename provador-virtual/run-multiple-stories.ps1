...
for ($i = $StartStory; $i -le $EndStory; $i++) {
    ...
    # Tenta primeiro padrão: 1.1-description.md, depois 1.1.description.md
    $storyPath = Join-Path $projectRoot "docs/stories"
    $storyFiles = @(Get-ChildItem -Path $storyPath -Filter "$storyId-*.md" -ErrorAction SilentlyContinue)
