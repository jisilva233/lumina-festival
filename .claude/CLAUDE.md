...
- **REGRA GLOBAL:** Sempre responder em Português Brasileiro (pt-BR)
- **Aplicação:** Todas as respostas, explicações, documentação, agentes e workflows
- **Exceção:** Apenas se usuário explicitamente solicitar outro idioma
- **Configuração:** `settings.json` → `"language": "portuguese"`
- **Persistência:** Cross-session (carregado automaticamente)

### Technical Language
- Nomes de variáveis: inglês (padrão técnico AIOS)
- Nomes de funções: inglês
- Comentários em código: português (quando apropriado)
- Output de ferramentas: respeita idioma original
