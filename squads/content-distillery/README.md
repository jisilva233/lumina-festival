...
**Activation:** `/content-distillery`

**Commands:** `*distill`, `*extract`, `*derive`, `*status`, `*agents`, `*gate`, `*context`, `*help`, `*exit`

**Pipeline Modes:**
- `*distill` - Full 6-phase pipeline (YouTube URL → frameworks → 60+ content)
- `*extract` - Framework extraction only (3 phases: ingest → extract → summarize)
- `*derive` - Content derivation only (3 phases: summarize → multiply → produce)

**Collaborate with:**
- `@devops` - Tool setup (yt-dlp, whisper, ffmpeg)
- `@dev` - Custom integrations and automation
- `@analyst` - Performance analysis and audience insights

**9 Specialist Agents:**
- Tier 0: tacit-extractor, model-identifier
- Tier 1: knowledge-architect, content-atomizer
- Tier 2: idea-multiplier, ecosystem-designer, production-ops
- Tier 3: youtube-strategist

**Quality Gates:**
- QG-001: Transcription Valid (advisory)
- QG-002: Extraction Complete (blocking)
- QG-003: Distillation Validated (blocking)
- QG-004: Content Reviewed (blocking)
